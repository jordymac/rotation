import { AudioMatchingEngine } from './audio-matching-engine';
...
/**
 * Audio Match Orchestrator
 * 
 * Manages the complete audio matching workflow with caching and persistence:
 * 1. Try Redis cache first (fastest)
 * 2. Try Postgres database (medium speed)  
 * 3. Compute fresh match using AudioMatchingEngine (slowest)
 * 4. Auto-approve and persist high-confidence matches
 */
export async function getOrComputeMatch(
  ...
  try {
    ...
    // 3. Compute fresh match using AudioMatchingEngine
    console.log(`[AudioMatchOrchestrator] Step 3: Computing fresh match using AudioMatchingEngine...`);
    ...
    // Use the modern AudioMatchingEngine with mix-aware intelligence
    const matchResult = await AudioMatchingEngine.findMatches(
      ...
      discogsVideos || [] // Always provide Discogs videos array (empty if none)
    );
...
export function createMatchService() {
  return {
    async findAudioMatches(
      ...
      try {
        // Use AudioMatchingEngine with context
        const matchResult = await AudioMatchingEngine.findMatches(
          releaseId,
          context.artist,
          [{
